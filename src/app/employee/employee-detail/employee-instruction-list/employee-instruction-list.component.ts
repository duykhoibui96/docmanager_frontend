...
@Component({
  selector: 'app-employee-instruction-list',
  templateUrl: './employee-instruction-list.component.html',
  styleUrls: ['./employee-instruction-list.component.css']
})
export class EmployeeInstructionListComponent extends SearchUtility implements OnInit {
  ...
  add() {
    ...
    this.conn.post("/employee-study/instructor/combine", {
  ...
  delete(item) {
    ...
    if (!confirm("Muốn xóa mục nghiên cứu khỏi danh sách hướng dẫn?"))
      return;

    this.conn.post("/employee-study/instructor/divide", {
